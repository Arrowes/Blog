...
## 模型权重和偏置初始化
优秀的初始化策略是模型稳定且快速收敛的基石。在现代卷积神经网络（如 3D BEV 感知、目标检测）中，初始化通常分为两部分：针对隐藏层权重（Weight）**的 Kaiming 初始化，以及针对**输出层偏置（Bias）的稀疏类先验初始化。

### 隐藏层权重：Kaiming (He) 初始化

**核心目标：** 解决深层网络中 ReLU 激活函数带来的方差衰减问题，防止梯度消失或爆炸。

早期的 Xavier (Glorot) 初始化假设激活函数是线性且关于原点对称的（如 Tanh）。但现代网络广泛使用 **ReLU** 激活函数：

$$f(x) = \max(0, x)$$

由于 ReLU 会将所有负数截断为 0，这会导致每经过一层网络，激活值的方差就会**减半**。如果不加干预，在极深的网络中，前向传播的信号会迅速衰减到 0，导致反向传播时梯度消失。

Kaiming 初始化（何恺明等人提出）通过在方差计算中引入系数 `2`，完美补偿了 ReLU 丢失的一半方差，使得网络每一层的输入和输出方差保持一致。

权重 $W$ 的采样分布分为两种：

* **正态分布 (Normal):** 均值为 0，方差为 $\frac{2}{n_{in}}$
...
*(注：$n_{in}$ 为该层神经元的输入连接数/通道数)*

* **适配激活函数：** ReLU, Leaky ReLU, PReLU 等非对称激活函数。
* **应用位置：** 卷积层（Conv2d/Conv3d）、全连接层（Linear）的 **Weight（权重）**。

### 输出层偏置：稀疏类先验偏置 (Prior Bias)

**核心目标：** 解决目标检测/BEV网格中极度不平衡的正负样本比例问题，防止训练初期 Loss 爆炸。

**-4.59 和 -2.19**

在 3D BEV 泊车网格中，绝大多数区域是空地（背景），障碍物（正样本）极其稀疏。如果使用默认的 0 初始化，最后一层 Logits 经过 Sigmoid 激活后，所有格点的初始预测概率都会是 **50%**，这会产生海量的误检（False Positives），导致初始 Loss 极高，破坏初期梯度方向。

该策略源自 **Focal Loss (RetinaNet)**，通过强制设定初始偏置，让模型在第一次前向传播时，输出的概率直接契合真实的物理分布先验。

设 $p$ 为我们期望的初始正样本先验概率。Logits 输出 $b$ 经过 Sigmoid 映射后的概率公式为：
...
| 目标类别 | 稀疏度假设 ($p$) | 偏置值 ($b$) 计算 | 配置文件中的设定值 |
| --- | --- | --- | --- |
| **车辆/行人 (极稀疏)** | 1% ($0.01$) | $-\ln(99) \approx -4.595$ | **-4.59** |
| **锥桶/关键点 (中等稀疏)** | 10% ($0.1$) | $-\ln(9) \approx -2.197$ | **-2.19** |

* **应用位置：** 网络最后的分类头（Classification Head）的 **Bias（偏置）**。
* **业务场景：** 3D 目标检测、BEV 占据栅格网络（Occupancy）、语义分割中极度不平衡的类别预测。

---

在一个标准的感知模型架构中，这两种初始化策略通常是**协同工作**的：

1. **骨干网/颈部网络 (Backbone/Neck):** 负责特征提取，使用 **Kaiming 初始化**处理权重，确保特征在经过几十层卷积和 ReLU 后依然保持良好的方差和梯度流动。
2. **检测头最后输出层 (Detection Head):** 负责最终分类，使用 **先验偏置 (-4.59)** 覆盖默认的 Bias，确保第一步输出的概率空间符合物理世界的稀疏规律，稳住训练的起步阶段。
