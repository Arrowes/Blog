...
### 模型调优
> 训练卡数、数据量、参数量、FLOPs、FP32/INT8 模型体积、端侧 FPS/latency、量化前后 mAP/IoU/召回率变化

+ Bad Case: 针对行人 KPI 偏低的问题，通过行人重采样、AI 合成数据、loss 调整（PIOU）和 LSS 架构调整，在当时的项目评估集与口径下，recall 从 60 提升到 80，precision 从 50 提升到 75。复用这些数字时必须同时注明数据版本、阈值和评估协议。
+ 轻量化设计：模型层：选 MobileNetV3、EfficientNet-Lite、CSPDarknet-nano、RepVGG/RepConv 等部署友好 backbone；
...
### 模型部署
> 项目指标数字准备不足：模型参数量、FLOPs、端侧延迟、模型体积、量化前后精度、优化缩减比例
...
### DMS
研究生阶段主要做 DMS 视觉检测，分两个任务。

#### 危险行为检测
目标是手机、水杯、双手离开方向盘等行为。难点是车内光照复杂，暗光和红外场景下纹理信息少。
方案: 数据和模型两端改进：数据端采集 RGB 与 IR 数据，并把单通道 IR 扩展为三通道，分别表示原始强度、对比度增强和边缘信息；模型端以 YOLOv7 为 baseline，加入 GAM 注意力增强关键区域特征，并做轻量化以适配嵌入式部署。

#### 疲劳检测
目标是眼睛/嘴巴开闭状态。难点是目标小、红外细节弱、需要时序判断。
方案: 在输入端用引导滤波增强细节，在模型端加入 Coordinate Attention 和小目标检测头，提高眼睛嘴巴检测召回；后处理端用 PERCLOS/PERMO 类时序指标统计连续帧闭眼和张嘴比例，判断疲劳程度。
