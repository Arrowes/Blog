...
> 项目评估曾把连续高度按 5 cm 区间离散统计。各区间的准确率必须结合样本量、范围定义和数据版本解读；中间微小高度是纯视觉测高的重点误差区间。
...
2. 模型侧: 
  ...
  + 2.2 BEV/LSS感知模型：输入是四路鱼眼图像，核心是FastBEV 特征转换，目标检测头在 BEV 空间输出车辆、行人、柱子、锥桶、骑行者等目标，同时还有多个检测头： FreeSpace实例分割、height map高度预测，车道线检测任务，其中我主要负责多任务目标检测、以及车道线检测。
    ...
    + 针对泊车场景的复杂线型，借鉴 GANet 将车道线建模为关键点估计与全局关联，通过预测关键点到所属车道起点的偏移实现关联；实际收益需与同一硬件和输入尺寸下的基线比较。
3. 工程侧: 参与 PyTorch 到 ONNX、量化、算子适配和平台部署问题定位。
   + ONNX 算子适配：`MatMul`、`ScatterND` 和稀疏算子的实际速度取决于芯片编译器、内存带宽和算子支持。应先查看目标平台支持表，再用端到端 profiling 选择投影实现，不能假定某一种算子在所有车载芯片上最快。
   + PTQ 量化：部分精细回归头对 INT8 量化误差较敏感。项目中采用主干 INT8、敏感任务头 FP16 的混合精度配置；“95%/5%”只描述当时模型的算力分布示例，需通过逐层误差分析和目标平台 profiling 重新确认。
---
