...
关键点任务可在真实位置周围生成二维高斯热力图作为监督目标。峰值对应关键点位置，周围响应随距离衰减；高斯宽度控制监督范围。这是标签构造方法，需要结合热力图回归或分类损失使用。
...
部分检测器会对同一目标生成多个预测框。标准贪心 NMS 按分数和重叠关系筛选，不保证保留下来的框定位最准确。多类别检测通常按类别分别执行，是否采用 class-agnostic NMS 取决于任务。
1. 按置信度排序：将所有预测框按置信度从高到低排序。选择最高置信度的框：取出当前列表中置信度最高的框，加入最终保留列表。
2. 计算IoU并抑制重叠框：计算该框与剩余所有框的交并比（IoU，Intersection over Union）。若某框与当前框的IoU超过设定的阈值（如0.5），则按抑制规则移除该候选框；拥挤场景中也可能误删不同目标。
3. 重复步骤1～2：对剩余框重复上述过程，直到所有框被处理。
IoU阈值：通常设为0.3\~0.7，控制框的重叠容忍度。阈值越低，抑制越严格。
置信度阈值：预过滤掉低置信度的框（例如只保留置信度≥0.5的框）。
标准实现参考 [Torchvision NMS](https://docs.pytorch.org/vision/stable/generated/torchvision.ops.nms.html)。

### 极坐标 NMS (Polar NMS)
> **项目实现记录，来源待补。** 下述椭圆模型、参数名与去重规则描述一种工程思路，尚未对应到可复现的仓库版本；应核验坐标系、相机标定和参数单位后再复用。

这里记录的 Polar NMS 面向**车载多相机环视系统 (Surround-View System, SVS)** 设计的一种几何感知型 NMS。其核心逻辑在于解决**单目相机测距的非对称不确定性**：
...
> **项目实现记录，来源待补。** `vshapely`、类别缩放系数和 IoU 阈值依赖具体实现，下面保留算法思路；示例参数不作为通用推荐值。
