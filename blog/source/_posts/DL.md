...
双线性插值是图像缩放中最常见的插值方法之一，核心思想是在二维平面上先沿一个方向做一次线性插值，再沿另一个方向再做一次线性插值。
...
工程里最容易踩坑的是 `align_corners`：
+ `align_corners=True`：输入输出角点严格对齐，坐标映射更“几何对齐”
+ `align_corners=False`：默认更常用，缩放行为更稳定，通常泛化更好

训练和推理阶段必须保持同样的 `align_corners` 设置，否则会出现细微的特征错位，分割或检测任务上会带来精度波动。
...
### 多任务损失权重策略
在多任务学习（Multi-Task Learning, MTL）中，常见总损失写法是：

$$
L_{total}=\sum_{i=1}^{T} w_i L_i
$$

其中 $L_i$ 是第 $i$ 个任务的损失，$w_i$ 是对应权重。难点在于不同任务的损失量级、收敛速度、噪声水平不同，固定权重往往不稳。因此常用动态加权方法，如 GradNorm 和 Uncertainty Loss。

#### GradNorm
GradNorm 的核心思想是：通过约束各任务在共享参数上的梯度范数，让不同任务以更平衡的训练速度共同优化，避免某个任务梯度过大主导训练。

设第 $i$ 个任务加权损失为 $w_iL_i$，其在共享层参数 $W$ 上的梯度范数为：

$$
G_i=\left\|\nabla_W(w_iL_i)\right\|_2
$$

GradNorm 会将 $G_i$ 拉向一个目标梯度强度，目标与任务相对训练速率相关，常见思想可写为：

$$
G_i^{*}=\bar{G}\cdot r_i^{\alpha}
$$

其中 $\bar{G}$ 为各任务梯度范数均值，$r_i$ 表示任务 $i$ 的相对训练速率，$\alpha$ 控制平衡强度（常见取值约 0.5）。

实践上，GradNorm 对多任务检测、分割、关键点等共享骨干网络场景很常用，优点是无需手工反复调权重；但实现时要额外计算梯度范数，训练开销会略有增加。

#### Uncertainty Loss
Uncertainty Loss（同方差不确定性加权）将任务权重建模为可学习的不确定性参数，让模型自动学习“哪个任务更可靠、该给多大权重”。
...
其中 $\sigma_i$ 是任务 $i$ 的可学习不确定性：
- 当任务噪声更大或更难时，模型倾向于学习更大的 $\sigma_i$，从而降低该任务损失权重
- $\log\sigma_i$ 项防止 $\sigma_i$ 无限制增大，保证优化稳定

工程上通常优化 $s_i=\log\sigma_i^2$ 来提升数值稳定性。该方法实现简单、训练稳定，在多任务回归+分类混合场景（如深度估计+语义分割）中非常常见。

两者对比：
- GradNorm：从梯度平衡角度动态调权，更关注各任务训练速率一致性
- Uncertainty Loss：从概率建模角度学习权重，更关注任务观测噪声与可信度
- 实践中可先用 Uncertainty Loss 快速起步，再根据训练曲线考虑是否切换或叠加梯度平衡策略
...
# Transformer
![图 13](https://raw.githubusercontent.com/Arrowes/Blog/main/images/DL13.png)  
![图 14](https://raw.githubusercontent.com/Arrowes/Blog/main/images/DL14.png)  
![图 15](https://raw.githubusercontent.com/Arrowes/Blog/main/images/DL15.png)  
