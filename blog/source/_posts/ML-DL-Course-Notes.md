...
配套阅读：[深度学习概念笔记](/DL/)、[PyTorch](/Pytorch/)、[训练经验](/DLtrain/)、[部署与量化](/DLdeploy/)。本文侧重 Andrew Ng 课程体系和公式复习，配套文章侧重专题原理、API 与工程实践。
...
本节用于说明从 RNN、门控单元到 Attention 的演进。多头注意力、Transformer Block、视觉 Transformer 和 DETR 的结构详见[深度学习概念笔记](/DL/#Transformer)。
...
- 优化器、学习率计划、梯度控制和常见训练策略见[深度学习概念笔记](/DL/)与[训练经验](/DLtrain/)；
- PyTorch 数据、模型保存、GPU 训练和常用 API 见[PyTorch 笔记](/Pytorch/)；
- ONNX、量化、剪枝、算子优化和部署验收见[部署与量化](/DLdeploy/)；
- BEV、车道线、多任务感知等具体视觉项目见[汽车与自动驾驶笔记](/Car/)。
